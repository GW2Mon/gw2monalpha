...
public class Equipment_has_ItemDao {
	...
	public ArrayList<Equipment_has_Item> getEquipment_has_Items() {
		...
		DB db = new DB();
		arrayList = (ArrayList<Equipment_has_Item>) db.session.createQuery(
				"from " + Equipment_has_Item.class).list();
		return arrayList;
	}
	...
	public ArrayList<Equipment_has_Item> getEquipment_has_Items(Object Param) {
		...
		if (Param.getClass() == Equipment.class)
			arrayList = (ArrayList<Equipment_has_Item>) db.session.createQuery(
					"from" + Equipment_has_Item.class + " where Equipment="
							+ ((Equipment) Param).getId()).list();
		else if (Param.getClass() == Item.class)
			arrayList = (ArrayList<Equipment_has_Item>) db.session.createQuery(
					"from" + Equipment_has_Item.class + " where Item="
							+ ((Item) Param).getId()).list();
		return arrayList;
	}
	...
	public Equipment_has_Item getEquipment_has_Item(Equipment Equip, Item item) {
		...
		DB db = new DB();
		Equip_has_Item = (Equipment_has_Item) db.session.createQuery(""
				+ Equipment_has_Item.class + "" + Equip.getId() + ""
				+ item.getId());
