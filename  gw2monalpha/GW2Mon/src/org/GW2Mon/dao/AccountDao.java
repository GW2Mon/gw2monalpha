...
public class AccountDao {
	...
	/**
	 * Returns Account with ID or Name Acc
	...
	public Account getAccount(Object Acc) {
		...
		DB db = new DB(GW2Mon.AccPath);
		if (Acc.getClass() == Integer.class)
			account = (Account) db.session.get(Account.class.getSimpleName(), (Integer) Acc);
		else
			if (Acc.getClass()==String.class)
			account = (Account) db.session.createQuery("from" + Account.class.getSimpleName()
					+ " where Name='" + Acc + "'");
			else System.out.println("Unsupported Type for Acc");
		return account;
	}
